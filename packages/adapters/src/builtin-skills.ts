...
const SKILLS: Array<{ name: string; description: string; body: string }> = [
  {
    ...
    body: `Use \`research_start\` for work that needs many sources, long reading, or a catalog sweep. Use \`web_search\` and \`web_fetch\` for a quick lookup.
1. Give a title, the goal, sources to prefer, success criteria and non-goals. Reference sources; do not paste them.
2. Do not run research through \`shell\`. Do not start a second job for the same goal while one runs or after an uncertain or timed-out result.
3. When woken, call \`research_status\`. Cite claims with their sources. Keep inference labeled. Report gaps.
4. Remember only durable conclusions, and name the research id as their source.`,
  },
  {
    name: "Interrogate",
