...
- Build Rakazo bot payloads with `botCreateInput`/`botSettingsPatch` in `apps/web/src/lib/bot-fields.ts`; a screen without an instructions editor must send no `instructions` field. The API still accepts `instructions` on update, so enforce this contract on every surface.
